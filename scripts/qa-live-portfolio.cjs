...
(async()=>{
 ...
 try{
  ...
  const cardsOverlap=switcherRect&&lastCardRect&&switcherRect.left<lastCardRect.x+lastCardRect.width&&switcherRect.left+switcherRect.width>lastCardRect.x&&switcherRect.top<lastCardRect.y+lastCardRect.height&&switcherRect.top+switcherRect.height>lastCardRect.y;
  ...
  if(switcherRect&&evidenceRect)assert.ok(evidenceRect.x+evidenceRect.width<=switcherRect.left-4,"desktop pager may not overlay Work evidence");
